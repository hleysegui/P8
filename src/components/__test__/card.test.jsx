...
import { APIContextProvider, useAPI } from "src/services/apiContext"
...
it('Fill card with properties info', () => {
    useAPI.fetchDatas = vi.fn().mockReturnValue(data)
    const id = data[0].id
    render(
        <MemoryRouter>
            <APIContextProvider value={data}>
                <Card title={data[0].title} key={id} cover={data[0].cover} />
            </APIContextProvider>
        </MemoryRouter>
    )
})